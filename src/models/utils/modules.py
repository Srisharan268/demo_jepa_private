...
#     def __init__(self, dim, num_heads, mlp_ratio=2.0, qkv_bias=False, act_layer=nn.GELU, norm_layer=nn.LayerNorm):
#         super().__init__()
#         self.norm1 = norm_layer(dim)
#         # self.normq = norm_layer(dim)  # Normalize query for stability
...
#         # y = self.xattn(self.normq(q), self.norm1(x))
#         q = q + y
...
class SpatialTemporalCrossAttention(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=1.0, qkv_bias=False, norm_layer=nn.LayerNorm):
        super().__init__()
        # We define only TWO blocks to handle everything. 
        # Total params approx: 2 blocks * ~7-8M (at 1408 dim) = ~15M total.
        
        # Block 1: Extracts motion/temporal cues from references
        self.temporal_block = CrossAttentionBlock(
            dim, num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias
        )
        
        # Block 2: Extracts spatial cues from references to current frame
        self.spatial_block = CrossAttentionBlock(
            dim, num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias
        )

        self.fuse_block = CrossAttentionBlock(
            dim, num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias
        )
    ...
    def forward(self, x, reference):
        B, N, D = x.shape
        num_samples = B // 2
        xt_seq = rearrange(x, '(b t) n d -> b (t n) d', b=num_samples, t=2)
        
        yt, yt_plus_1 = torch.chunk(reference, 2, dim=0)
        yt_seq = rearrange(yt, '(b t) n d -> b (t n) d', b=num_samples, t=2)
        yt_plus_1_seq = rearrange(yt_plus_1, '(b t) n d -> b (t n) d', b=num_samples, t=2)

        yt_motion = self.temporal_block(yt_plus_1_seq, yt_seq)

        x_spatial = self.spatial_block(xt_seq, yt_seq)

        x_fused = self.fuse_block(x_spatial, yt_motion)

        x_fused = rearrange(x_fused, 'b (t n) d -> (b t) n d', t=2)
...
class LoRALinear(nn.Module):
    """
    Replaces a dense dim*dim matrix with two smaller matrices:
    (dim -> rank) and (rank -> dim).
    
    Params savings:
    Standard: dim * out_dim
    LoRA:     (dim * rank) + (rank * out_dim)
    """
    def __init__(self, in_dim, out_dim, rank=16, bias=True):
        super().__init__()
        self.lora_down = nn.Linear(in_dim, rank, bias=False)
        self.lora_up = nn.Linear(rank, out_dim, bias=bias)
        self.rank = rank
    ...
    def forward(self, x):
        # x: [Batch, Tokens, Dim] -> [Batch, Tokens, Rank] -> [Batch, Tokens, Out_Dim]
        return self.lora_up(self.lora_down(x))

class LoRAMLP(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.0, lora_rank=16):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        self.fc1 = LoRALinear(in_features, hidden_features, rank=lora_rank, bias=True)
        self.act = act_layer()
        self.fc2 = LoRALinear(hidden_features, out_features, rank=lora_rank, bias=True)
        self.drop = nn.Dropout(drop)
...
class LoRACrossAttention(nn.Module):
    def __init__(self, dim, num_heads=12, qkv_bias=False, use_sdpa=True, lora_rank=16):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = head_dim**-0.5
        ...
        # --- LoRA Modification ---
        # Instead of nn.Linear(dim, dim), we use LoRA factorization
        self.q = LoRALinear(dim, dim, rank=lora_rank, bias=qkv_bias)
        
        # For KV, we project to 2*dim. 
        # LoRA path: dim -> rank -> 2*dim
        self.kv = LoRALinear(dim, int(dim * 2), rank=lora_rank, bias=qkv_bias)
        # -------------------------
    ...
    def forward(self, q, x):
        B, n, C = q.shape
        # q projection uses LoRA
        q = self.q(q).reshape(B, n, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)

        B, N, C = x.shape
        # kv projection uses LoRA
        kv = self.kv(x).reshape(B, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
...
class LoRACrossAttentionBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=2.0, qkv_bias=False, 
                 lora_rank=16, act_layer=nn.GELU, norm_layer=nn.LayerNorm):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.normq = norm_layer(dim)  # Normalize query for stability
        
        # Use the LoRA version of CrossAttention
        self.xattn = LoRACrossAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, lora_rank=lora_rank)
        
        self.norm2 = norm_layer(dim)
        
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = LoRAMLP(in_features=dim, hidden_features=mlp_hidden_dim, out_features=dim, act_layer=act_layer, lora_rank=lora_rank)
...
class SpatialTemporalCrossAttentionLoRA(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=2.0, qkv_bias=False, lora_rank=32, norm_layer=nn.LayerNorm):
        super().__init__()
        
        # Block 1: Extracts motion/temporal cues from references
        self.temporal_block = LoRACrossAttentionBlock(
            dim, num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, lora_rank=lora_rank, norm_layer=norm_layer
        )
        
        # Block 2: Extracts spatial cues from references to current frame
        self.spatial_block = LoRACrossAttentionBlock(
            dim, num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, lora_rank=lora_rank, norm_layer=norm_layer
        )

        # Block 3: Fuses spatial and temporal information
        self.fuse_block = LoRACrossAttentionBlock(
            dim, num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, lora_rank=lora_rank, norm_layer=norm_layer
        )
...
class Expert(nn.Module):
    """
    Expert module with one cross attention block and several self attention blocks.
    If temporal, adds temporal positional embeddings to distinguish between two samples.
    """
    def __init__(
        self,
        dim,
        ...
        **kwargs,
        ...
        # Temporal positional embeddings (only used if is_temporal=True)
        if self.is_temporal:
            self.temporal_pos_emb_0 = nn.Parameter(torch.zeros(1, 1, dim))
            self.temporal_pos_emb_1 = nn.Parameter(torch.zeros(1, 1, dim))
            # # Initialize with small values
            # nn.init.normal_(self.temporal_pos_emb_0, std=0.01)
            # nn.init.normal_(self.temporal_pos_emb_1, std=0.01)
    ...
    def forward(self, x0, x1):
        """
        Args:
            x: Input tensor of shape [2*B, N, D] containing two samples (concatenated along batch dim)
        ...
        if self.is_temporal:
            ...
            x0 = x0 + self.temporal_pos_emb_0
            x1 = x1 + self.temporal_pos_emb_1
            
        x = self.cross_attn_block(x0, x1)
        
        # Self attention blocks
        for block in self.self_attn_blocks:
            x = block(x)
        
        # # Apply output normalization for stability
