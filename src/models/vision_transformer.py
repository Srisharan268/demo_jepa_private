...
from src.models.utils.modules import Block, CrossAttentionBlock, SpatialTemporalCrossAttention, SpatialTemporalCrossAttentionLoRA
...
class VisionTransformerDreamer(nn.Module):
    """Vision Transformer using cross attention adapter as a dreamer"""
    ...
    def __init__(
        ...
        use_rope=False,
        handle_nonsquare_inputs=True,
        adapter_type="ca", # stca or ca or stca-lora
        lora_rank=32,
        **kwargs
    ):
        super().__init__()
    ...
    def load_adapter(self, adapter_path = None):

        if self.adapter_type == "ca":
            self.adapter = [CrossAttentionBlock(dim=self.embed_dim, num_heads=self.num_heads, qkv_bias=self.qkv_bias) \
                for _ in range(self.depth)]
        elif self.adapter_type == "stca":
            self.adapter = [SpatialTemporalCrossAttention(dim=self.embed_dim, num_heads=self.num_heads, qkv_bias=self.qkv_bias) \
                for _ in range(self.depth)]
        elif self.adapter_type == "stca-lora":
            self.adapter = [SpatialTemporalCrossAttentionLoRA(dim=self.embed_dim, num_heads=self.num_heads, qkv_bias=self.qkv_bias, lora_rank=self.lora_rank) \
                for _ in range(self.depth)]
        else:
            raise ValueError(f"Invalid adapter type: {self.adapter_type}")
        # self.adapter = [CrossAttentionBlock(dim=self.embed_dim, num_heads=self.num_heads, qkv_bias=self.qkv_bias) \
        #      for _ in range(self.depth)]
        self.adapter = nn.ModuleList(self.adapter)
    ...
    def forward(self, x, masks=None, reference=None):
        """
        :param x: input image/video
        :param masks: indices of patch tokens to mask (remove)
        :param reference: reference image/video to be used as a dreamer, same type as x, we do per-layer cross attention
        """
        if reference is None:
            raise ValueError("Reference is required for dreamer.\n \
                Please provide a reference image/video or turn to VisionTransformer.")

        #assert x.shape == reference.shape, "Input and reference must have the same shape"
        if masks is not None and not isinstance(masks, list):
            masks = [masks]
        ...
        if not self.use_rope:
            pos_embed = self.interpolate_pos_encoding(x, self.pos_embed)
            ref_pos_embed = self.interpolate_pos_encoding(reference, self.pos_embed)
            x = self.patch_embed(x)
            x += pos_embed
            reference = self.patch_embed(reference)
            reference += ref_pos_embed
        else:
            x = self.patch_embed(x)
            reference = self.patch_embed(reference)
        ...
        for i, blk in enumerate(self.blocks):
            if self.use_activation_checkpointing:
                ...
                reference = torch.utils.checkpoint.checkpoint(
                    blk, reference, masks, None, T=T, H_patches=H_patches, W_patches=W_patches, use_reentrant=False
                )
            else:
                x = blk(x, mask=masks, attn_mask=None, T=T, H_patches=H_patches, W_patches=W_patches)
                reference = blk(reference, mask=masks, attn_mask=None, T=T, H_patches=H_patches, W_patches=W_patches)
            # if i < self.depth - 1:
            if i < self.depth:
                x = self.adapter[i](x, reference)
            if self.out_layers is not None and i in self.out_layers:
                outs.append(self.norm(x))
    ...
    #     """
    #     Freeze all parameters except for adapter parameters created by load_adapter.
        
    #     Returns:
    #         list: List of parameters that are not frozen (adapter parameters)
    #     """
    #     # Freeze all model parameters first
    #     for param in self.parameters():
    #         param.requires_grad = False
    #     # Collect and unfreeze adapter parameters if they exist
    #     unfrozen_params = []
    ...
    def freeze_encoder(self, unfreeze_vit = False):
        """
        Freeze all parameters except for adapter parameters.
        Returns the list of trainable adapter parameters.
        """
        unfrozen_vit_params = []
        unfrozen_adapter_params = []
        if unfreeze_vit:
            for name, param in self.named_parameters():
                param.requires_grad = True
                if "adapter" in name:
                    unfrozen_adapter_params.append(param)
                else:
        ...
        return unfrozen_vit_params, unfrozen_adapter_params
...
def vit_dreamer_giant_xformers(patch_size=16, **kwargs):
    model = VisionTransformerDreamer(
        patch_size=patch_size,
        embed_dim=1408,
        depth=40,
        num_heads=22,
