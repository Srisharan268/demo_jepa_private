...
def cem(
    ...
    :param only_xyz: if True, only xyz is sampled by CEM; rpy/gripper action channels are fixed
        to externally supplied GT values via ``gt_rpy_action`` / ``gt_gripper_action``.
    :param gt_rpy_action: GT rpy delta action used when ``only_xyz=True``. Tensor/array of shape
        (3,) or (rollout, 3). Defaults to zeros (no rotation) if None.
    :param gt_gripper_action: GT gripper action used when ``only_xyz=True``. Scalar or shape
        (rollout,). Required when ``only_xyz=True`` and ``fixed_gripper`` is not set.
        Semantics follow ``abs_gripper``: absolute target in [0, 1] if ``abs_gripper=True``,
        otherwise a delta on current gripper.
    """
    if only_xyz and not use_rpy:
        raise ValueError("cem(only_xyz=True) requires use_rpy=True.")
    ...
    if only_xyz:
        if gt_rpy_action is None:
            gt_rpy_action_t = torch.zeros((rollout, 3), device=context_frame.device, dtype=torch.float32)
        else:
            gt_rpy_action_t = torch.as_tensor(
                gt_rpy_action, device=context_frame.device, dtype=torch.float32
            )
            if gt_rpy_action_t.dim() == 1:
                if gt_rpy_action_t.numel() != 3:
                    raise ValueError(f"gt_rpy_action must have 3 elements, got shape {tuple(gt_rpy_action_t.shape)}.")
                gt_rpy_action_t = gt_rpy_action_t.expand(rollout, 3).contiguous()
            elif gt_rpy_action_t.shape != (rollout, 3):
                raise ValueError(
                    f"gt_rpy_action must be (3,) or ({rollout}, 3), got {tuple(gt_rpy_action_t.shape)}."
                )

        if fixed_gripper is None:
            if gt_gripper_action is None:
                raise ValueError("cem(only_xyz=True) requires gt_gripper_action when fixed_gripper is None.")
            gt_gripper_action_t = torch.as_tensor(
                gt_gripper_action, device=context_frame.device, dtype=torch.float32
            ).flatten()
            if gt_gripper_action_t.numel() == 1:
                gt_gripper_action_t = gt_gripper_action_t.expand(rollout).contiguous()
            elif gt_gripper_action_t.numel() != rollout:
                raise ValueError(
                    f"gt_gripper_action must be a scalar or have {rollout} elements, "
                    f"got {tuple(gt_gripper_action_t.shape)}."
                )
        else:
            gt_gripper_action_t = None
    else:
        gt_rpy_action_t = None
        gt_gripper_action_t = None
    ...
    if only_xyz:
        mean[:, 3:6] = gt_rpy_action_t
        std[:, 3:6] = 0.0
    if fixed_gripper is not None:
        mean[:, -1] = fixed_gripper
        std[:, -1] = 0.0
    elif only_xyz:
        mean[:, -1] = gt_gripper_action_t
        std[:, -1] = 0.0
    ...
    def sample_action_traj():
        ...
        for h in range(rollout):
            ...
            if only_xyz:
                action_samples[:, 3:6] = gt_rpy_action_t[h]
                if fixed_gripper is None:
                    action_samples[:, -1] = gt_gripper_action_t[h]
            ...
            elif fixed_gripper is not None:
...
class WorldModel(object):
    ...
    def __init__(
        ...
        # If True: CEM only samples xyz deltas; rpy/gripper action channels are filled with GT
        # values supplied at __call__ time via gt_rpy_action / gt_gripper_action.
    ...
    def __call__(
        ...
        if self.only_xyz and gt_rpy_action is None:
            raise ValueError(
                "WorldModel(only_xyz=True) requires gt_rpy_action to be passed to __call__."
            )
        if self.only_xyz and gt_gripper_action is None and not self.discrete_gripper:
            raise ValueError(
                "WorldModel(only_xyz=True) requires gt_gripper_action to be passed to __call__ "
        ...
        only_xyz_kwargs = {}
        if self.only_xyz:
            only_xyz_kwargs["only_xyz"] = True
            only_xyz_kwargs["gt_rpy_action"] = gt_rpy_action
            if not self.discrete_gripper:
                only_xyz_kwargs["gt_gripper_action"] = gt_gripper_action
    ...
    def dummy_test(
        ...
        if self.only_xyz and (gt_rpy_action is None or gt_gripper_action is None):
            raise ValueError(
                "WorldModel(only_xyz=True).dummy_test requires gt_rpy_action and gt_gripper_action."
            )
        current_rep = self.encode(current_img)
