...
from app.vjepa_2_1_dreamer_ac.cem_utils import WorldModel
...
def build_reference_loader(params: dict) -> ReferenceLoader:
    ...
    image_key = deploy_cfg.get("image_key", "observations/images/right_shoulder_rgb")
...
def build_mpc_args(deploy_cfg: dict) -> dict:
    mpc_cfg = deploy_cfg.get("mpc", {})

    return {
        "rollout": int(mpc_cfg.get("rollout", 1)),
        "samples": int(mpc_cfg.get("samples", 200)),
        "topk": int(mpc_cfg.get("topk", 10)),
        "cem_steps": int(mpc_cfg.get("cem_steps", 50)),
        "momentum_mean": float(mpc_cfg.get("momentum_mean", 0.15)),
...
def build_world_model(args: dict):
    ...
    if compile_model:
        logger.info("Compiling encoder, predictor, target_encoder, and dreamer_predictor.")
    ...
    world_model = WorldModel(
        ...
        transform=transform,
        mpc_args=build_mpc_args(deploy_cfg),
...
def format_action(action) -> list:
    ...
    if action.ndim != 1:
        raise ValueError(f"Expected action to be 1-D or 2-D, got shape={action.shape}")
...
def deploy_loop(params: dict, debugmode: bool = False) -> None:
    ...
    deploy_cfg = params.get("deploy", {})
    l1_threshold = float(deploy_cfg.get("l1_threshold", 0.28))
