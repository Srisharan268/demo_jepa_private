...
_PAIRED_H5_DOMAIN_PRESETS = {
    "sim": {"primary_subdir": "franka", "reference_subdir": "sawyer", "episode_fps": 30},
    "real": {"primary_subdir": "franka", "reference_subdir": "ur", "episode_fps": 5},
    "sim2real": {"primary_subdir": "real", "reference_subdir": "sim", "episode_fps": 5},
}
...
def init_data(
    ...
    ``episode_fps``: optional override of the HDF5 source rate used in ``ceil(episode_fps / fps)``.
        For ``sim``, the original loader always used 30 Hz; that is fixed here and ``episode_fps`` is ignored.
    """
    ...
    logger.info(
        "dreamer_ac init_data: data_type=%s primary=%s reference=%s",
        data_type,
