...
from app.vjepa_2_1_imitation.dataset import init_data
...
def main(args, resume_preempt=False):
    ...
    # -- DIFFUSION HEAD
    cfgs_diffusion = cfgs_model.get("diffusion_head")
    ...
    # L2 grad clip for diffusion_head; set to 0 or null in yaml to disable
    grad_clip_max_norm = cfgs_opt.get("grad_clip_max_norm", 1.0)
    ...
    diffusion_head = DistributedDataParallel(diffusion_head, find_unused_parameters=True)
    logger.info("Diffusion head has been wrapped with DDP.")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr in range(ipe):
            ...
            def load_clips():
                # franka_clips = sample[0].to(device, non_blocking=True)  # [B C T H W]
                # varianced_sawyer_clips, sawyer_clips = sample[1]  # [B C T H W], [B C T H W]
                # varianced_sawyer_clips = varianced_sawyer_clips.to(device, non_blocking=True)
                # sawyer_clips = sawyer_clips.to(device, non_blocking=True)
                # actions = sample[2].to(device, dtype=torch.float, non_blocking=True)  # [B T-1 7]
                # states = sample[3].to(device, dtype=torch.float, non_blocking=True)  # [B T 7]
                primary_images, reference_images, primary_actions, primary_states = sample
                if use_extrinsics:
                    extrinsics = sample[4].to(device, dtype=torch.float, non_blocking=True)  # [B T 7]
                else:
                    extrinsics = None
                return (primary_images.to(device, non_blocking=True), reference_images.to(device, non_blocking=True), primary_actions.to(device, dtype=torch.float, non_blocking=True), primary_states.to(device, dtype=torch.float, non_blocking=True), extrinsics)
            ...
            def train_step():
                ...
                def forward_diffusion_head(cond, actions):
                    cond = cond.reshape(-1, tokens_per_frame, cond.shape[-1])
                    actions = actions.reshape(-1, 1, actions.shape[-1])
                    return diffusion_head(cond, actions)
                ...
                with torch.cuda.amp.autocast(dtype=dtype, enabled=mixed_precision):
                    ...
                    loss = forward_diffusion_head(dreamer_target_feature, actions)
