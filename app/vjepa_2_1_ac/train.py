...
from app.vjepa_2_1_ac.dataset import init_data
from app.vjepa_2_1_ac.transforms import make_transforms
from app.vjepa_2_1_ac.utils import (
    init_opt,
    init_video_model,
    load_checkpoint,
    load_pretrained,
    vjepa_droid_encoder_args_from_cfg,
)
...
def main(args, resume_preempt=False):
    ...
    if rank == 0:
        wandb.init(project="vjepa_2_1_ac", name=f"vjepa_2_1_ac")
    else:
    ...
    encoder, predictor = init_video_model(
        ...
        **vjepa_droid_encoder_args_from_cfg(cfgs_model),
    )
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for itr in range(ipe):
            ...
            # print(f"clips: {clips.shape}, actions: {actions.shape}, states: {states.shape}")
            # continue
            ...
            def train_step():
                ...
                def forward_target(c):
                    with torch.no_grad():
                        # Per-frame micro-clips: [B,C,T,H,W] -> [B*T,C,tubelet,H,W] so each real frame
                        # forms one tubelet step (matches tokens_per_frame per timestep). tubelet_size
                        # must match the ViT patch encoder (same as v2 AC).
                        c = (
                            c.permute(0, 2, 1, 3, 4)
                            .flatten(0, 1)
                            .unsqueeze(2)
                        ...
                        # V-JEPA 2.1 ViT: training=False -> norms_block[-1] (embed_dim); training=True
                        # would return hierarchical concat (4 * embed_dim) and break the predictor.
